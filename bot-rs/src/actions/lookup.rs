...
use crate::models::{Deck, User, Match};
use super::league::current_league;
use anyhow::Result;

pub fn lookup_user(conn: &PgConnection, user: &SerenityUser) -> Result<User> {
    use crate::schema::users::dsl::*;
    users.filter(discordid.eq(*user.id.as_u64() as i64)).get_result(conn).map_err(|e| e.into())
}
...
pub fn lookup_deck(conn: &PgConnection, user: &SerenityUser) -> Result<Option<Deck>> {
    ...
    decks.filter(owner.eq(user.id).and(league.eq(lg.id)).and(active.eq(true)))
        .get_result(conn).optional().map_err(|e| e.into())
}

/// Looks up the match between the two given users from the current league.
pub fn lookup_match(conn: &PgConnection, left: &SerenityUser, right: &SerenityUser) -> Result<Option<Match>> {
    ...
    let query = 
        matches.filter(winning_deck.eq(uleft.id).and(losing_deck.eq(uright.id)).or(winning_deck.eq(uright.id).and(losing_deck.eq(uleft.id))));
