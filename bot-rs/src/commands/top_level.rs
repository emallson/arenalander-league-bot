...
fn league(ctx: &mut Context, msg: &Message) -> CommandResult {
    ...
    let message = match league {
        Some(league) => format!("The {} League is currently active. It began on {} and runs until the end of {} UTC. To register a deck for the league, use `!register`. League standings can be viewed at {}/standings", league.title, league.start_date.format("%e %B %Y"), (league.end_date - Duration::seconds(1)).format("%e %B %Y"), BASE_URL),
