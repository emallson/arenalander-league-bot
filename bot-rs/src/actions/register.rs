...
pub fn register_deck(conn: &PgConnection, user: &SerenityUser, parsed_deck: ParsedDeck) -> Result<Deck> {
    ...
    let active_deck: Option<Deck> = decks.filter(league.eq(current_league.id).and(owner.eq(user.id)).and(active.eq(true)))
        .first(conn).optional()?;
    ...
    let new_deck: Deck = insert_into(decks)
        .values((league.eq(current_league.id), owner.eq(user.id), creation_date.eq(now)))
        .get_result(conn)?;
    let contents = parsed_deck.into_iter()
        .map(|entry| (deck.eq(new_deck.id), card.eq(entry.uuid), count.eq(entry.count as i32)))
        .collect::<Vec<_>>();
...
pub fn resign(conn: &PgConnection, user: &SerenityUser) -> Result<()> {
    let user: User = users.filter(discordid.eq(*user.id.as_u64() as i64)).get_result(conn)?;
    let current_league = get_league(conn)?;
    if current_league.is_none() {
        return Err(RegistrationError::NoLeague.into());
    }
    ...
    update(decks).filter(owner.eq(user.id).and(league.eq(current_league.id)))
